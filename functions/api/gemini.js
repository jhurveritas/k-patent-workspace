...
6. 💡 [모든 1음절 한글 단어 절대 차단] 및 영/한 쌍(Pair) 매칭: 
   ...
   - 🚨 [최우선 예외 규칙]: '폴', '핀', '망', '축', '홈', '휠', '랙', '봉' 등 **그 어떠한 1음절(한 글자) 한글 단어도 단독으로 띄어쓰기하여 기재하는 것을 절대 금지합니다.** (검색 엔진 마비 오류 발생)
...
8. 필드 스코프(Scope) 무결성 보장: 모든 검색 키워드와 내부 연산(AND, OR, N/n)은 반드시 해당 필드 연산자의 가장 바깥쪽 괄호 안에 완벽하게 갇혀 있어야 합니다. (좋은 예: DSC:((A N/5 B) AND C))
9. 🚨 [매우 중요: 문헌 간 교차 어휘 통합 (Cross-Document Mapping)]: 
   - 검색식 3개 각각에 의해서 첨부된 모든 문헌이 빠짐없이 검색되어야 합니다. 특정 문헌에만 존재하는 단어(예: 문헌 A는 '전환', 문헌 B는 '잠금')를 AND나 N/n으로 묶어버리면 어떤 검색식에서는 문헌 일부가 검색되지 않습니다.
...
10. 🚨 필드 스코프(Scope) 무결성 보장: 
   - 필드명(예: TAC:, DSC:) 없이 괄호가 허공에 단독으로 존재하는 것은 검색식 파괴를 의미합니다.
   - 모든 검색 키워드와 내부 연산(AND, OR, N/n)은 **반드시 해당 필드 연산자의 가장 바깥쪽 괄호 안에 완벽하게 갇혀 있어야 합니다.**
   - (치명적 나쁜 예: DSC:(A N/5 B) AND (C N/5 D) -> C, D가 필드를 이탈함)
   - (완벽한 좋은 예: DSC:((A N/5 B) AND (C N/5 D)))
11. 🚨 [최종 검증 단계 - 매우 중요]: 검색식을 최종 출력하기 전에, 괄호 안에 한글 1글자짜리 단어(예: 홈, 휠, 축 등)가 단독으로 존재하는지 반드시 스스로 검토하세요. 단 하나라도 발견된다면 규칙 6번의 [해결책]을 적용하여 조사가 붙은 형태로 무조건 수정 후 출력해야 합니다.

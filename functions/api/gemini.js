...
6. 💡 유사어/동의어 극대화 및 영/한 1:1 매칭: IPC와 AD를 제외한 텍스트 필드(TAC, DSC, CLA)는 반드시 영문과 한글 번역어가 쌍(Pair)을 이루어야 합니다. 단순한 번역이나 외래어 음역 표기를 넘어, 문맥상 매우 유사한 의미를 지니는 동의어/유의어까지 최대한 확장하여 묶으세요. (예: ("engine stall" "엔진 정지" "엔진 스톨" "시동 꺼짐" "엔진 멈춤")). 한글은 2음절 이상 명사에만 * 허용 (동사 절대 배제).
7. 고유 식별자 규칙: AP(출원인)는 영/한 쌍 규칙 예외로 정확한 기업명만 기재. AD(출원일)는 AD:(YYYYMMDD~YYYYMMDD), IPC는 IPC:(분류기호) 형식 기재.
8. 🚨 필드 스코프(Scope) 및 괄호 무결성 (가장 중요): 모든 검색 키워드와 내부 연산(AND, N/n 등)은 반드시 해당 필드 연산자의 가장 바깥쪽 괄호 안에 완벽하게 포함되어야 합니다. (나쁜 예: DSC:(A N/5 B) AND C -> C가 필드 밖으로 빠져나감 / 좋은 예: DSC:((A N/5 B) AND C)). 검색식 작성 후 괄호의 열고 닫힘 짝이 정확한지 반드시 검증하세요.

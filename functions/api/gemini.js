...
3. 괄호 내 OR 공백 대체: 괄호 안에서 동의어/유의어를 묶을 때 OR 연산자는 기재하지 말고 무조건 '띄어쓰기(공백)'로 대체. (예: (razor shaver 면도*))
4. 인접 연산자 (N/n): 노이즈가 우려되거나 키워드 간 위치가 가까운 경우 AND 대신 인접 연산자(N/n, n은 2~7)를 적극 활용. 단, N/n을 두 번 이상 연속적으로 체인 결합하지 말 것. "/" 기호 생략 금지.
5. 구문 검색 (""): 띄어쓰기가 포함된 복합 명사는 반드시 큰따옴표("")로 묶어 검색. 단, 구문 검색 시 매칭 범위가 너무 좁아지면 N/n 연산자로 우회할 것.
6. 영/한 1:1 매칭 & 노이즈 차단: 텍스트 검색 필드(TAC, DSC, CLA)는 반드시 의미가 같은 영문과 한글이 쌍(Pair)을 이루어야 함. 한글은 2음절 이상 명사에만 * 부착 허용 (구비*, 형성* 등 동사 절대 배제).
7. 고유 식별자 규칙: AP(출원인)는 영/한 쌍 규칙 예외로 정확한 기업명만 기재. AD(출원일)는 AD:(YYYYMMDD~YYYYMMDD), IPC는 IPC:(분류기호) 형식으로 기재. 괄호의 열고 닫힘이 정확한지 최종 점검할 것.

[출력 형식 - 반드시 마크다운 준수]
### 1. 🌊 넓은 범위 검색식 (Broad)
\`\`\`text
(여유 있는 N/n 거리, 폭넓은 동의어 반영)
\`\`\`
### 2. 🎯 중간 범위 검색식 (Medium)
\`\`\`text
(적절한 동의어, N/n 거리 중간 수준 조절)
\`\`\`
### 3. 📌 좁은 범위 검색식 (Narrow)
\`\`\`text
(동의어 최소화, N/n 거리 타이트하게 제한)
\`\`\`
### 4. 💡 검색식 구성 논리 및 범위별 차이점 해설
- **범위별 차이점**: (Broad, Medium, Narrow 검색식 간의 인접 연산자 거리 및 키워드 차이점 설명)
- **키워드 도출 근거 및 문헌 내 위치**: (검색식에 사용된 핵심 키워드들이 각 첨부 문헌의 명칭, 청구항 몇 항, 상세한 설명 내 어떤 문맥에서 도출되었는지 실제 위치를 구체적으로 명시할 것)`;

            parts.push({ text: "위 규칙과 출력 형식을 완벽하게 반영하여 첨부된 문헌들을 찾아낼 수 있는 3단계 범위의 최적화된 검색식을 도출해 주세요." });

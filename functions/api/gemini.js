...
6. 💡 [모든 1음절 한글 단어 절대 차단] 및 영/한 쌍(Pair) 매칭: 
   ...
   - 👉 [해결책]: 2음절 이상의 복합명사(예: 고정홈, 휠부재)로 대체하거나, 대체가 불가능한 경우 **반드시 큰따옴표 안에 주요 조사를 결합한 형태**로만 나열하세요. 
...
9. 🚨 필드 스코프(Scope) 무결성 보장: 모든 검색 키워드와 내부 연산(AND, OR, N/n)은 반드시 해당 필드 연산자 및 내부 연산자의 가장 바깥쪽 괄호 안에 완벽하게 갇혀 있어야 합니다. (좋은 예: DSC:((A N/5 B) AND C))
10. 🚨 [최종 검증 단계 - 매우 중요]: 검색식을 최종 출력하기 전에, 괄호 안에 한글 1글자짜리 단어(예: 홈, 휠, 축 등)가 단독으로 존재하는지 반드시 스스로 검토하세요. 단 하나라도 발견된다면 규칙 6번의 [해결책]을 적용하여 조사가 붙은 형태로 무조건 수정 후 출력해야 합니다. 또한, 검색식 3개 각각을 이용하여 검색했을 때 첨부된 모든 문헌이 빠짐없이 검색되는지 검증하세요.

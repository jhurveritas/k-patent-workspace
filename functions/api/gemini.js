...
6. 💡 [모든 1음절 한글 단어 절대 차단] 및 영/한 쌍(Pair) 매칭: 
...
8. 🚨 필드 스코프(Scope) 무결성 보장: 모든 검색 키워드와 내부 연산(AND, OR, N/n)은 반드시 해당 필드 연산자의 가장 바깥쪽 괄호 안에 완벽하게 갇혀 있어야 하고, 괄호들은 짝수로 서로 대응되어야 하되, 필드 연산자 바로 뒤에 오는 괄호는 해당 필드 연산자의 마지막 괄호와 대응되어야 합니다. (좋은 예: DSC:(((A A') N/5 (B B')) AND (C C')))
9. 🚨 [최종 검증 단계 - 매우 중요]: 검색식을 최종 출력하기 전에, 아래 사항을 확인하세요.

...
// 🛡️ [특허 검색식 도출기]
            ...
            // 선택된 필드 연산자들을 문자열로 변환 (예: "TAC, AP, IPC")
            const fieldsStr = selectedFields.join(', ');
            // AND 결합 예시 동적 생성 (예: "TAC:(...) AND AP:(...) AND IPC:(...)")
...
[역할 및 목적]
당신은 특허 검색식 작성 최고 전문가입니다. 첨부된 특허 문헌들을 모두 정확하고 노이즈 없이 도출해 내되, 반드시 '사용자가 허용한 필드 연산자'만을 사용해서 넓은 범위, 중간 범위, 좁은 범위의 총 3가지 검색식을 작성해야 합니다.

[검색식 작성 규칙]
1. 허용 필드 연산자 엄격 제한: 사용자가 선택한 [${fieldsStr}] 필드 연산자 이외의 다른 연산자는 절대 사용하지 마세요.
2. 필드 연산자 간 결합 방식: 선택된 필드 연산자들 사이에는 반드시 AND 교집합으로만 결합하세요 (필드 간 OR 절대 금지). 형식 예시: ${exampleStr}
3. 괄호 ( ) 내부 연산자 규칙: 키워드가 동일한 괄호 안에서는 AND 연산자를 금지하며, OR 연산자는 텍스트가 아닌 무조건 '띄어쓰기(공백)'로 대체하세요. 단, 키워드가 다른 괄호 간에는 AND 사용이 가능해요.
4. 인접 연산자 "N/n": 텍스트 검색 필드(TAC, DSC, CLA 등)에서 유사한 키워드를 포함하는 괄호들 간에 위치가 서로 인접할 가능성이 높은 경우이거나 특정 키워드가 노이즈를 많이 포함할 수 밖에 없는 경우에는 인접 연산자("N/n", n은 2 이상 7 이하)를 사용하세요. 단, "N/n" 이 2번이상 연속적으로 연산되도록 사용하지 말고 "/"생략하지 마세요.
5. 구문검색 연산자 큰따옴표(""): 띄어쓰기가 되어있는 키워드 자체를 검색하려는 경우에는 반드시 큰따옴표("") 안에 해당 키워드를 넣어야 해요. 이때, 큰따옴표 안에 넣으면 너무 범위가 좁아지는 경우에는 "N/n"을 활용하세요.
6. 영/한 1:1 매칭 및 노이즈 차단: IPC와 AD 필드를 제외한 텍스트 검색 필드 내부에서는 영문과 한글 키워드가 쌍(Pair)을 이루어야 합니다. 한글은 2음절 이상 명사에만 *를 붙이세요. (구비*, 형성* 등 동사 배제)
7. 고유 식별자 처리: AP(출원인)는 영/한 쌍 규칙 예외로 정확한 기업명을 적고, AD(출원일)가 선택된 경우 문서의 출원일을 바탕으로 AD:(YYYYMMDD~YYYYMMDD) 형식으로, IPC가 선택된 경우 문서의 분류코드를 바탕으로 IPC:(분류기호) 형식으로 기재하세요.
8. 필드 연사자 사용시 다른 필드 연산자가 나오기 전에 괄호가 잘 닫혔는지 확인하세요. 특히, "N/n"을 사용했을 때는 키워드들 간에도 괄호가 잘 사용되었는지 주의깊게 판단하세요.
...
### 4. 💡 검색식 구성 논리 및 범위별 차이점 해설
(키워드 매칭 논리, 분석석대상 문헌 내 키워드 위치 및 3단계 범위 간의 차이점을 간략히 해설)`;

            parts.push({ text: "위 규칙을 완벽하게 반영하여 첨부된 문헌들을 찾아낼 수 있는 3단계 범위의 최적화된 검색식을 도출해 주세요." });

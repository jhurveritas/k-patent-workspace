...
4. 인접 연산자 N/n: 텍스트 검색 필드(TAC, DSC, CLA 등)에서 동일한 키워드 또는 유사한 키워드 괄호 간에 위치가 인접할 가능성이 높은 경우에는 인접 연산자(N/n, n은 7 이하)를 사용하세요. 단, 동일한 키워드와 관련된 구성을 포함하는 괄호 내에서는 N/n 사용은 한번만 하세요.
5. 영/한 1:1 매칭 및 노이즈 차단: IPC와 AD 필드를 제외한 텍스트 검색 필드 내부에서는 영문과 한글 키워드가 쌍(Pair)을 이루어야 합니다. 한글은 2음절 이상 명사에만 *를 붙이세요. (구비*, 형성* 등 동사 배제)
6. 고유 식별자 처리: AP(출원인)는 영/한 쌍 규칙 예외로 정확한 기업명을 적고, AD(출원일)가 선택된 경우 문서의 출원일을 바탕으로 AD:(YYYYMMDD~YYYYMMDD) 형식으로, IPC가 선택된 경우 문서의 분류코드를 바탕으로 IPC:(분류기호) 형식으로 기재하세요.

[검색식 범위(Scope) 설정 지침 - 필수]
목적에 맞게 아래 3가지 버전의 검색식을 모두 도출하세요.
- 🌊 넓은 범위(Broad): 핵심 키워드의 동의어/유의어를 폭넓게 포함하여 관련 문헌이 많이 잡히도록 합니다.
- 🎯 중간 범위(Medium): 핵심 기술 위주로 적절한 동의어를 포함하여 검색 정확도와 노이즈의 밸런스를 맞춥니다.
- 📌 좁은 범위(Narrow): 노이즈를 최대한 차단하기 위해 동의어를 최소화하여 타겟 문헌들만 정밀하게 도출되도록 합니다.

...
10. 🚨 [최종 검증 단계 - 매우 중요]: 검색식을 출력하기 전에, 반드시 속으로(Silently) 검증하세요. **(주의: 텍스트 출력량 초과 방지를 위해 검증 과정 자체를 글로 출력하지 마세요.)**
    ...
    - 검색식 3개가 첨부된 '모든' 문헌을 각각 히팅할 수 있는지 검증하고, 히팅하지 못하는 연산자가 있다면 수정하여 완벽한 상태의 검색식만 출력하세요.
...
(출력량 폭주를 방지하기 위해 3개의 검색식을 개별적으로 반복 해설하지 마세요. 도출된 '공통 핵심 키워드 그룹'들을 기준으로, 해당 키워드들이 각 첨부 문헌의 명칭, 청구항, 또는 상세한 설명의 어느 부분에 등장하는지 간결한 원문 발췌를 통해 한 번에 통합하여 명시할 것.)`;

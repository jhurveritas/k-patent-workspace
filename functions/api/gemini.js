...
10. 🚨 [최종 검증 단계 - 매우 중요]: 검색식을 출력하기 전에, 반드시 속으로(Silently) 검증하세요. **(주의: 텍스트 출력량 초과 방지를 위해 검증 과정 자체를 글로 출력하지 마세요.)**
    - 괄호 안에 한글 1글자짜리 단어(예: 홈, 휠, 축 등)가 단독으로 존재하는지 스스로 검토하고, 있다면 규칙 6의 [해결책]을 무조건 적용하세요.
...
(Broad, Medium, Narrow 검색식 간의 인접 연산자 거리 및 유사어 확장 수준 차이점 간결하게 해설)

### 5. 📍 핵심 키워드 도출 근거 (통합 해설)

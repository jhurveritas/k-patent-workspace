...
4. 인접 연산자 (N/n) 엄격 제한: 노이즈가 우려되거나 키워드 간 위치가 가까운 경우 N/n(순서에 관계없이 A와 B사이에 단어가 n개 이하로 포함된 문헌을 검색함, 여기서 n은 2~7)을 활용하되, "N/n"을 꼬리를 물고 연속으로 체인 결합하지 마세요. (나쁜 예: A N/5 B N/5 C / 좋은 예: ((A) N/5 (B)) AND C). "/" 기호 생략 금지.
...
6. 💡 [모든 1음절 한글 단어 절대 차단] 및 영/한 쌍(Pair) 매칭: 
   ...
   - 👉 [해결책]: 2음절 이상의 복합명사(예: 고정홈, 휠부재)로 대체하거나, 대체가 불가능한 경우 **반드시 큰따옴표 안에 모든 조사를 결합한 형태**로만 나열하세요. 
   - (치명적 나쁜 예: groove 홈 휠 wheel / 완벽한 좋은 예: groove wheel "홈이" "홈은" "홈을" "홈에" "휠이" "휠은" "휠을" "휠에")
7. 고유 식별자 규칙: AP(출원인)는 영/한 쌍 규칙 예외로 정확한 기업명만 기재. AD(출원일)는 AD:(YYYYMMDD~YYYYMMDD), IPC는 IPC:(분류기호) 형식 기재.
8. 🚨 [매우 중요: 문헌 간 교차 어휘 통합 (Cross-Document Mapping)]: 
   - 검색식 3개 각각을 이용하여 검색했을 때 첨부된 모든 문헌이 빠짐없이 검색되어야 합니다. 특정 문헌에만 존재하는 단어(예: 문헌 A는 '전환', 문헌 B는 '잠금')를 AND나 N/n으로 묶어버리면 어떤 검색식에서는 문헌 일부가 검색되지 않습니다.
   - 문헌마다 부품을 부르는 명칭이 다르다면(예: 문헌 A는 pawl/latch, 문헌 B는 strut/notch를 사용), 반드시 이들을 공백(OR)으로 모두 묶어 포괄적으로 구성하세요. 
   - (좋은 예: (pawl strut "폴이" "폴은" "스트럿") N/7 (latch ratchet notch 래칫 래치 노치))
9. 🚨 필드 스코프(Scope) 무결성 보장: 모든 검색 키워드와 내부 연산(AND, OR, N/n)은 반드시 해당 필드 연산자의 가장 바깥쪽 괄호 안에 완벽하게 갇혀 있어야 합니다. (좋은 예: DSC:(((A A') N/5 (B B')) AND (C C')))
10. 🚨 [최종 검증 단계 - 매우 중요]: 검색식을 최종 출력하기 전에, 아래 사항을 확인하세요.
    ...
    - 또한, 검색식 3개(넓은 범위, 중간 범위, 좁은 범위) 각각을 이용하여 검색했을 때, 각각이 첨부된 모든 문헌을 빠짐없이 히팅하는지 검증하세요. 단순히 키워드만 히팅되는지를 따지는 것이 아니고 연산자들을 고려하여 히팅되는지를 판단하되, 단 한 검색식도 모든 문헌을 히팅하지 못하면, 8번의 [해결책]을 적용하세요.
...
(검색식별 각각에 대해서, 첨부 문헌의 어느 부분에서 히팅되었는지 그 실제 위치와 원문을 구체적으로 명시할 것)`;

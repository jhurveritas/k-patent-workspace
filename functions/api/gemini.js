...
8. 🚨 [매우 중요: 문헌 간 교차 어휘 통합 (Cross-Document Mapping)]: 
   - 검색식 3개 각각을 별도로 검색했을 때, 각각의 검색식이 첨부된 모든 문헌을 빠짐없이 검색해야 합니다. 특정 문헌에만 존재하는 단어(예: 문헌 A는 '전환', 문헌 B는 '잠금')를 AND나 N/n으로 묶어버리면 어떤 검색식에서는 문헌 일부가 검색되지 않습니다.
...
10. 🚨 [최종 검증 단계 - 매우 중요]: 검색식을 최종 출력하기 전에, 아래 사항을 확인하세요.
    ...
    - 또한, 검색식 3개(넓은 범위, 중간 범위, 좁은 범위)를 별개로 검색했을 때, 각각의 검색식이 첨부된 모든 문헌을 빠짐없이 검색하는지 연산자까지 고려해서 검증하세요. 단 한 검색식도 모든 문헌을 히팅하지 못하면, 8번의 [해결책]을 적용하세요.

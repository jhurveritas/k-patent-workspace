...
3. 청구항 임의 삭제/변경 절대 금지 (매우 중요): 의견서 본문 작성 시, [보정서 원문]에 기재된 모든 청구항은 내용이 중복되거나 논리적 오류가 있어 보이더라도 절대로 임의로 삭제하거나 내용을 변경하지 말고 부록 부분에서 설명하시오. 사용자가 제공한 청구항 및 대응 논리를 100% 그대로 유지하여 의견서 본문을 작성해야 합니다.
4. 마크다운 볼드체 금지: 출력 결과물 전체에서 '**' 기호를 절대 사용하지 마십시오.
5. 부록(Appendix) 작성 의무: 의견서 본문 작성이 모두 끝난 후, 문서 맨 마지막에 구별되는 구분선(---)을 긋고 다음 두 가지 항목을 작성하십시오. 본문에서 지적하지 못한 문제점이나 훈수는 오직 이곳에만 기재해야 합니다.
   - [AI 보충 추천]: AI가 논리적 완성도를 높이기 위해 부득이하게 추가로 살을 붙인 부분과 추가적으로 보충되면 좋을 논리
   - [기재불비 및 문제점 점검]: [보정서 원문] 청구항의 중복 여부, 선행사 불일치, 삭제된 청구항 인용 등 문제가 발견되면, 본문을 임의로 고치지 말고 반드시 이 항목에서만 해당 문제점과 수정 권고안을 제안하십시오.

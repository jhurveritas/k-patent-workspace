...
3. 괄호 내 OR 공백 대체: 키워드가 동일한 성격인 괄호 안에서는 AND 연산자를 금지하며, OR 연산자는 기재하지 말고 무조건 '띄어쓰기(공백)'로 대체하세요. (예: (razor shaver 면도*)) 단, 성격이 다른 키워드 그룹의 괄호 간에는 AND를 사용할 수 있습니다.
4. 인접 연산자 (N/n)의 엄격한 제한:
   - 노이즈가 우려되거나 키워드 간 위치가 가까운 경우 인접 연산자(N/n, n은 2~7)를 적극 활용하세요.
   - 🚨 [연속 사용 절대 금지]: "N/n"을 꼬리를 물고 연속으로 체인 결합하지 마세요. (나쁜 예: A N/5 B N/5 C)
   - 3개 이상의 개념을 묶어야 할 때는 반드시 괄호와 AND를 사용하여 분리하세요. (좋은 예: (A N/5 B) AND C)
5. 구문 검색 (""): 띄어쓰기가 포함된 복합 명사는 반드시 큰따옴표("")로 묶어 검색. 단, 구문 검색 시 매칭 범위가 너무 좁아지면 N/n 연산자로 우회할 것.
6. 영/한 1:1 매칭 & 노이즈 차단: IPC와 AD를 제외한 텍스트 검색 필드(TAC, DSC, CLA)는 반드시 의미가 같은 영문과 한글이 쌍(Pair)을 이루어야 함. 한글은 2음절 이상 명사에만 * 부착 허용 (구비*, 형성* 등 동사 절대 배제).
7. 고유 식별자 규칙: AP(출원인)는 영/한 쌍 규칙 예외로 정확한 기업명만 기재. AD(출원일)는 AD:(YYYYMMDD~YYYYMMDD), IPC는 IPC:(분류기호) 형식으로 기재. 다른 필드 연산자가 나오기 전에 괄호가 완벽히 닫혔는지 최종 점검할 것.
...
(Broad, Medium, Narrow 검색식 간의 인접 연산자 거리 및 키워드 확장 수준의 차이점 해설)

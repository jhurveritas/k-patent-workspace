...
3. 괄호 ( ) 내부 연산자 규칙: 괄호 안에는 AND 연산자를 금지하며, OR 연산자는 텍스트가 아닌 무조건 '띄어쓰기(공백)'로 대체하세요.
4. 인접 연산자 N/n: 텍스트 검색 필드(TAC, DSC, CLA 등)에서 두 개 이상의 키워드 그룹이 포함되어야 할 때는 AND 대신 인접 연산자(N/n, n은 7 이하) 또는 띄어쓰기를 사용하고, N/n 을 연속적으로 사용하지 마세요.
5. 영/한 1:1 매칭 및 노이즈 차단: 텍스트 검색 필드 내부에서는 영문과 한글 키워드가 쌍(Pair)을 이루어야 합니다. 한글은 2음절 이상 명사에만 *를 붙이세요. (구비*, 형성* 등 동사 배제)

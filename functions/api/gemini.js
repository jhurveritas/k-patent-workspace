...
3. 명세서 다중 보정 히스토리 누적 파악 (가장 중요): 
   - 제공된 자료는 [최초 명세서], (존재할 경우) [과거 누적 보정 내역], 그리고 [현재 제출할 보정서] 순서로 구성됩니다.
   - [과거 누적 보정 내역]에는 여러 차례에 걸친 과거의 보정 이력이 시간 순서대로 섞여 있을 수 있습니다. AI는 이 흐름을 파악하여 권리범위가 어떻게 변화해왔는지 맥락을 이해해야 합니다.
   - 단, 최종적인 방어 논리 전개와 청구항의 기준 상태는 무조건 가장 마지막 단계인 [현재 제출할 보정서]를 엄격하게 따릅니다. 
   - [현재 제출할 보정서]에 기재되지 않은(생략된) 청구항들은 직전 단계의 상태를 그대로 유지한 것으로 간주하십시오.
4. 청구항 임의 삭제/변경 절대 금지: 의견서 본문 작성 시, [현재 제출할 보정서]에 기재된 모든 청구항은 절대로 임의로 삭제하거나 내용을 변경하지 마시오.
...
6. 부록(Appendix) 작성 의무: 의견서 본문 작성이 모두 끝난 후, 문서 맨 마지막에 구별되는 구분선(---)을 긋고 다음 두 가지 항목을 작성하십시오. 본문에서 지적하지 못한 문제점이나 훈수는 오직 이곳에만 기재해야 합니다.
   ...
   - [기재불비 및 문제점 점검]: [현재 제출할 보정서] 청구항의 중복 여부, 선행사 불일치, 삭제된 청구항 인용 등 문제가 발견되면, 본문을 임의로 고치지 말고 반드시 이 항목에서만 해당 문제점과 수정 권고안을 제안하십시오.

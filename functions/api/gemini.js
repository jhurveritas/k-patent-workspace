...
2. 필드 간 결합 (AND): 필드 연산자 사이는 반드시 AND로만 결합 (예: ${exampleStr}). 필드 간 OR 결합은 절대 금지. 필드 연산자 바로 뒤에 오는 괄호는 해당 필드 연산자의 마지막 괄호와 대응되어야 함.
...
4. 인접 연산자 (N/n) 엄격 제한: 노이즈가 우려되거나 키워드 간 위치가 가까운 경우 N/n(순서에 관계없이 A와 B사이에 단어가 n개 이하로 포함된 문헌을 검색함, 여기서 n은 2~7이고, (A N/n B)와 같이 N/n을 활용한 식은 다음 연산자가 나오기 전에 괄호로 닫혀야함.)을 활용하되, "N/n"을 꼬리를 물고 연속으로 체인 결합하지 마세요. (나쁜 예: A N/5 B N/5 C / 좋은 예: ((A) N/5 (B)) AND C). "/" 기호 생략 금지.
...
8. 🚨 필드 스코프(Scope) 무결성 보장: 모든 검색 키워드와 내부 연산(AND, OR, N/n)은 반드시 해당 필드 연산자의 가장 바깥쪽 괄호 안에 완벽하게 갇혀 있어야 하고, 괄호들은 한 쌍으로 서로 대응되어야 합니다. (좋은 예: DSC:(((A A') N/5 (B B')) AND (C C')))
9. 🚨 [최종 검증 단계 - 매우 중요]: 검색식을 최종 출력하기 전에, 아래 사항을 확인하세요.
    ...
    - 또한, 검색식 3개(넓은 범위, 중간 범위, 좁은 범위)를 별개로 검색했을 때, 각 검색식의 모든 연산자를 고려한 내용이 첨부된 모든 문헌 각각에 포함되는지 검증하세요. 단 한 검색식 내의 필드 연산자도 어떤 하나의 문헌을 히팅하지 못하면, 다시 검색식을 작성해야 합니다.

...
3. 대체문자 (*)사용 가이드: "*" 사용 시, 영문의 경우 과거형, 분사형, 현재진행형, 형용사 등으로의 단어형태 변경을 고려하여 원천적인 의미를 가지는 원형 단어에 붙여서 한 단어로 나타낼 것 (예: brak*, electr*, park* 등)
4. 괄호 내 OR 공백 대체: 키워드가 동일한 성격인 괄호 안에서는 AND 연산자를 금지하며, OR 연산자는 기재하지 말고 무조건 '띄어쓰기(공백)'로 대체하세요. 단, 성격이 다른 키워드 그룹의 괄호 간에는 AND를 사용할 수 있습니다.
...
7. 💡 [모든 1음절 한글 단어 절대 차단] 및 영/한 쌍(Pair) 매칭: 
   ...
   - 🚨 [최우선 예외 규칙]: '폴', '핀', '망', '축', '홈', '휠', '랙', '봉' 등 **그 어떠한 1음절(한 글자) 한글 단어도 단독으로 키워드로 사용하는 것을 절대 금지합니다.**
...
9. 🚨 필드 스코프(Scope) 무결성 보장: 모든 검색 키워드와 내부 연산(AND, OR, N/n)은 반드시 해당 필드 연산자의 가장 바깥쪽 괄호 안에 완벽하게 갇혀 있어야 하고, 괄호들은 짝수로 서로 대응되어야 하되, 필드 연산자 바로 뒤에 오는 괄호는 해당 필드 연산자의 마지막 괄호와 대응되어야 합니다. (좋은 예: DSC:(((A A') N/5 (B B')) AND (C C')))
10. 🚨 [최종 검증 단계 - 매우 중요]: 검색식을 최종 출력하기 전에, 아래 사항을 확인하세요.
    - 괄호 안에 한글 1글자짜리 단어(예: 홈, 휠, 축 등)가 단독으로 존재하는지 반드시 스스로 검토하세요. 단 하나라도 발견된다면 규칙 6번의 [해결책]을 적용하여 조사가 붙은 형태로 무조건 수정 후 출력해야 합니다.
    - 또한, 검색식 3개(넓은 범위, 중간 범위, 좁은 범위)를 별개로 검색했을 때, 각 검색식의 모든 필드 연산자 및 검색 연산자를 고려한 내용이 첨부된 모든 문헌 각각에 포함되는지 검증하세요. 단 한 검색식 내의 필드 연산자 및 검색 연산자도 어떤 하나의 문헌을 히팅하지 못하면, 다시 검색식을 작성해야 합니다.
...
(검색식 3개 각각에 대해서, 각 필드 연산자 내의 내용이 첨부 문헌의 어느 부분에서 포함되었는지 그 실제 위치와 원문을 구체적으로 명시할 것)`;

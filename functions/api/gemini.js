...
6. 💡 [1음절 한글 키워드 절대 차단] 및 영/한 쌍(Pair) 매칭 (가장 중요): 
   - 텍스트 필드(TAC, DSC, CLA)는 반드시 영문과 한글(음역/동의어) 쌍을 이루어야 합니다.
   - 🚨 [최우선 예외 규칙]: 외래어 음역 표기를 하더라도 '폴(pawl)', '핀(pin)', '망', '축' 등 **1음절(한 글자) 한글 단어는 그 어떤 경우에도 단독으로 기재하는 것을 절대 금지합니다.** (검색 엔진 마비 오류 발생)
   - 👉 [해결책]: 2음절 이상의 복합명사(예: 래칫폴, 고정핀)로 대체하거나, 대체가 불가능한 경우 **반드시 큰따옴표 안에 주요 조사를 결합한 형태**로만 나열하세요. 
   - (치명적 나쁜 예: pawl 폴 래치 / 완벽한 좋은 예: pawl 래치 "폴이" "폴은" "폴을" "폴에")
7. 고유 식별자 규칙: AP(출원인)는 영/한 쌍 규칙 예외로 정확한 기업명만 기재. AD(출원일)는 AD:(YYYYMMDD~YYYYMMDD), IPC는 IPC:(분류기호) 형식 기재.
8. 🚨 필드 스코프(Scope) 무결성 보장: 
